...
import { fetchLoans, fetchLoan } from "../api/loanApi";
...
export default function LoanReviewForm({ onSubmit, loading }) {
  ...
  useEffect(() => {
      fetchLoans()
      .then(setLoans)
      .catch((err) => { console.error("fetchLoans failed:", err); });
  }, []);
  ...
  async function handleScenarioSelect(e) {
    ...
    try {
      ...
      setForm({
        ...loan,
        question: SCENARIO_QUESTIONS[loanId] || "",
      });
    } catch {
      // keep current form if fetch fails
    }
  }
  ...
  return (
    <form className="card form-card" onSubmit={handleSubmit}>
      ...
      <div className="grid two">
        ...
        <label>
          Borrower Name
          <input name="borrower_name" value={form.borrower_name} onChange={handleChange} required />
        </label>
        <label>
          Loan Type
        ...
        <label>
          Annual Income
          <input name="annual_income" type="number" value={form.annual_income} onChange={handleChange} required />
        </label>
        <label>
          Credit Score
          <input name="credit_score" type="number" value={form.credit_score} onChange={handleChange} required />
        </label>
        <label>
          Submitted Documents
          <input
            value={form.submitted_documents.map((d) => d.doc_type).join(", ") || "none"}
            readOnly
            style={{ color: "#888" }}
          />
        </label>
      ...
      <button disabled={loading} type="submit" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
        {loading && (
          <span style={{
