...
class DocumentRequiredLookupTool:
    ...
    async def call(self, args: dict) -> dict:
        """
        Hardcoded required-doc lookup (Option A).
        """
        ...
        required = required_docs_map.get(loan_type, [])
        return {"required_documents": required}
...
class LoanApplicationLookupTool:
    ...
    async def call(self, args: dict) -> dict:
        """
        Fetch one LoanApplicationTable row by primary key.
        """
        loan_id = args["loan_id"]
        
        with Session(engine) as session:
            app = session.get(LoanApplicationTable, loan_id)
            if app is None:
...
class SubmittedDocumentsLookupTool:
    ...
    async def call(self, args: dict) -> dict:
        """
        Query SubmittedDocumentTable for all rows matching loan_id.
        """
        loan_id = args["loan_id"]
